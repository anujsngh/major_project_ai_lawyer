...
def load_preprocessed_docs(documents):
    return [preprocess_text(text=doc) for doc in documents]


def load_tokenized_docs(documents):
    return [word_tokenize(text=doc.lower()) for doc in documents]
...
def store_word2vev_model(corpus):
    ...
    model.save("trained_models/word2vec_model.bin")
    return model


def load_word2vev_model():
    # Load the pre-trained Word2Vec model from a file
    model = Word2Vec.load("trained_models/word2vec_model.bin")
    return model
...
def store_TF_IDF_feature_vectors(preprocessed_query, preprocessed_docs):
    ...
    save_npz('trained_models/tfidf_matrix.npz', tfidf_matrix)

    # Save query_tfidf_matrix to disk in NPZ format
    save_npz('trained_models/query_tfidf_matrix.npz', query_tfidf_matrix)

    return query_tfidf_matrix, tfidf_matrix


def load_TF_IDF_feature_vectors():
    tfidf_matrix = load_npz('trained_models/tfidf_matrix.npz')
    query_tfidf_matrix = load_npz('trained_models/query_tfidf_matrix.npz')
    return query_tfidf_matrix, tfidf_matrix
