...
def rel_search(query, doc_type):
    ...
    try:
        ...
        preprocessed_docs = store_preprocessed_docs(documents=documents, doc_type=doc_type)
    
    preprocessed_query = preprocess_text(text=query)

    try:
        query_tfidf_matrix, doc_tfidf_matrix = load_doc_tfidf_matrix(preprocessed_query, doc_type=doc_type)
    except:
        query_tfidf_matrix, doc_tfidf_matrix = store_doc_tfidf_matrix(preprocessed_query, preprocessed_docs, doc_type=doc_type)

    similarity_scores = compute_cosine_similarity_score(query_tfidf_matrix, doc_tfidf_matrix)
...
def rel_statutes_search(query):
    ...
    top_docs.append("**********************************")
    top_docs.append(rel_statutes_search1(query))
    return top_docs
...
if __name__ == "__main__":
    ...
    top_documents_json = rel_cases_search(query)
